    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if action_name == "navigated":
                communicator.update_json_field_py(key=key, field_path="current_position", new_value=list(kwargs.values())[0])
            elif action_name == "grasped":
                communicator.update_json_field_py(key=key, field_path="robot_holding", new_value=list(kwargs.values())[0])
            elif action_name == "placed":
                communicator.update_json_field_py(key=key, field_path="robot_holding", new_value=[])
            return result
        return wrapper
    return decorator